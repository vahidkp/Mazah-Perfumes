    ...
    <section className="max-w-3xl mx-auto px-6 py-12 md:py-16">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-3">
            ...
            {step > 0 && (
              <button
                onClick={back}
                className="inline-flex items-center gap-1 font-body text-xs text-muted hover:text-charcoal transition-colors"
              >
                <ArrowLeft size={14} /> Back
              </button>
            )}
          </div>
          <div className="h-1 rounded-pill bg-charcoal/10 overflow-hidden">
            <motion.div
      ...
      <AnimatePresence mode="wait">
        {!done ? (
          ...
          >
            <h2 className="font-display text-3xl md:text-4xl text-charcoal font-bold mb-8 text-center">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {QUESTIONS[step].options.map((opt) => (
                <button
                  key={opt.label}
                  onClick={() => choose(opt)}
                  className="text-left p-6 rounded-card border border-charcoal/15 bg-white hover:border-gold-primary hover:shadow-card transition-all duration-200 group"
                >
                  <p className="font-heading text-xl text-charcoal group-hover:text-gold-primary transition-colors">
                    {opt.label}
                  </p>
                  <p className="font-body text-sm text-muted mt-1">{opt.desc}</p>
                </button>
              ))}
            </div>
          </motion.div>
          ...
          >
            ...
            <h2 className="font-display text-3xl md:text-4xl text-charcoal font-bold mb-2">
              We found your scent
            </h2>
            <p className="font-body text-sm text-muted mb-10 max-w-md mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center mt-10">
              ...
              <button
                onClick={restart}
                className="inline-flex items-center justify-center gap-2 rounded-pill border border-charcoal/20 text-charcoal px-8 py-3.5 font-body text-sm tracking-widest uppercase hover:border-gold-primary hover:text-gold-primary transition-colors"
              >
